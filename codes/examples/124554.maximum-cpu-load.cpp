...
public:
    int findMaxCPULoad(std::vector<Job>& jobs)
    {
        ...
        for (int i = 0; i < jobs.size(); i++) {
            result.push_back({jobs[i].start, jobs[i].cpuload});
            result.push_back({jobs[i].end, -jobs[i].cpuload});
        }
    ...
    int findMaxCPULoad1(std::vector<Job>& jobs)
    {
        ...
        int size = jobs.size();
        auto cmp = [](Job &a, Job &b){return a.end > b.end;};
        // 以结束时间排序在pq中，用于先结束先出堆；
        int sum = 0;
        int max = std::numeric_limits<int>::min();
        ...
        for (int i = 0; i < size; ++i) {
            pq.push(jobs[i]);
            sum += jobs[i].cpuload;
            while (!pq.empty() && pq.top().end < jobs[i].start) {
                sum -= pq.top().cpuload;
                pq.pop();
            }
            max = std::max(max, sum);
