...
public:
    ...
    std::vector<std::vector<int>> employeeFreeTime1(std::vector<std::vector<std::vector<int>>>& schedule)
    {
        ...
        for (int i = 1; i< temp.size(); ++i) {
            if (result.back()[1] < temp[i][0]) {
                ...
                result.back()[1] = std::max(temp[i][1], result.back()[1]);
            }
        }
        std::cout << result << std::endl;
        for (int i=0; i < result.size(); ++i) {
            if (i+1< result.size() && result[i][1] < result[i+1][0]) {
                result_.push_back({result[i][1] , result[i+1][0]});
            }
        }
        return result_;
        // merge
    ...
    std::vector<std::pair<int,int>> employeeFreeTime0(std::vector<std::vector<std::pair<int,int>>>& schedule)
    {
        ...
        for (auto x: schedule) {
            for (auto y: x) {
                result.push_back({y.first, 1});
                result.push_back({y.second,-1});
            }
        }
        sort(result.begin(), result.end());
        int prev = -1;
        ...
        int sum = 0;
        for (auto x: result) {
            if (sum == 0 && prev != -1 && x.first != prev) {
                res.push_back({prev, x.first});
            }
            sum += x.second;
            prev = x.first;
        }
