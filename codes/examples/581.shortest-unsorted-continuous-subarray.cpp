...
class Solution {
public:
    int findUnsortedSubarray0(std::vector<int>& nums)
    {
        int size = nums.size();
        int left = 0;
        ...
        while (right < size && nums[right] < max) {
            ++right;
        }
        std::cout << "["  << left << "]" << nums[left] << " [" << right << "]" << nums[right] << std::endl;
        return right -left+1;
    ...
    int findUnsortedSubarray(std::vector<int>& nums)
    {
        ...
        int right = -1;
        for (i =0; i < size; ++i) {
            std::cout << "maxn:" << maxn << " nums[" << i << "]" << nums[i] << std::endl;
            if (maxn > nums[i]) {
        ...
        std::cout << " maxn:" << maxn << " nums[" << right << "]" << nums[right] << std::endl;
        for (int j = size-1; j >= 0; j--) {
            if (minn < nums[j]) {
                left = j;
            } else {
