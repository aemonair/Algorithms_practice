...
public:
    ...
    std::vector<int> findSubstring0(std::string s, std::vector<std::string>& words)
    {
        ...
        for (left = 0; left < s.size() - count*size+1; left++) {
            auto umap_(umap);
            match = 0;
            std::string str = s.substr(left, size);
            std::cout << str << std::endl;
            if (umap.count(str)> 0) {
                match =1;
                umap_[str]--;
                printunordered_map(umap_);
                for (int i =1; i < count; i++) {
                    std::string str_ = s.substr(left+size*i, size);
                    std::cout <<" str_" << str_ << std::endl;
                    if (umap_.count(str_) < 0) {
                        break;
                    }
                    umap_[str_] --;
                    if (umap_[str_] >= 0) {
