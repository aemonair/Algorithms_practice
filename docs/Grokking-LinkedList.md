
[206.reverse-linked-list_反转链表](<leetcode/206.reverse-linked-list_反转链表.md>)

[092.reverse-first-k_反转链表前k个](<leetcode/092.reverse-first-k_反转链表前k个.md>)

[092.reverse-linked-list-ii_反转链表II](<leetcode/092.reverse-linked-list-ii_反转链表II.md>)

[025.reverse-nodes-in-k-group_K个一组翻转链表](<leetcode/025.reverse-nodes-in-k-group_K个一组翻转链表.md>)

[025.reverse-alternating-nodes-in-k-group_间隔K个一组翻转链表](<leetcode/025.reverse-alternating-nodes-in-k-group_间隔K个一组翻转链表.md>)

[061.rotate-list_旋转链表](<leetcode/061.rotate-list_旋转链表.md>)

#LinkedList;
